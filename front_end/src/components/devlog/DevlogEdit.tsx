...
const DevlogEdit = () => {
  ...
  const fetchDevlog = async () => {
    try {
      setLoadingDevlog(true);
      const response = await api.get(`/devlogs/${id}`);
      const devlog = response.data;

      const fullCoverUrl = getFullImageUrl(devlog.coverImage);
      setExistingCoverImage(fullCoverUrl);
      setCoverImagePreview(fullCoverUrl);

      setTitle(devlog.title);
      setIsPublished(devlog.isPublished);

      if (devlog.originalNotebook) {
        setSelectedNotebookId(devlog.originalNotebook.id.toString());
      }

      if (devlog.content) {
        try {
          const parsed = JSON.parse(devlog.content);
          const fixed = parsed.map((block: any) => {
            if (block.type === 'image' && block.src) {
              return { ...block, src: getFullImageUrl(block.src) };
            }
            return block;
          });
          setContent(
      ...
      toast.error('Devlog yüklenirken hata oluştu');
      navigate('/devlogs');
    } finally {
      setLoadingDevlog(false);
    }
  };
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      await api.put(`/devlogs/${id}`, devlogData);
      toast.success('Devlog başarıyla güncellendi!');
      navigate(`/devlogs/${id}`);
    } catch (error: any) {
      console.error('Devlog update error:', error);
      toast.error(
        error.response?.data?.message || 'Devlog güncellenirken hata oluştu'
      );
    } finally {
      setIsLoading(false);
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <Card>
            <div className="flex justify-between items-center mb-6">
              ...
              <Button color="light" onClick={() => navigate(`/devlogs/${id}`)}>
                İptal
              </Button>
            </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-4">
                    ...
                    <p className="text-sm text-gray-500">
                      • Yeni dosya yüklerseniz mevcut görsel değiştirilir
                      <br />
                      • Maksimum dosya boyutu: 10MB
                      <br />• Boş bırakırsanız mevcut görsel korunur
                    </p>
                  </div>
                </div>
        ...
        <div className="p-6">
          <div className="text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              Devlog sayfasını güncellemek istediğinize emin misiniz?
            </h3>
